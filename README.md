...
- Largest detected face controls the eyes and, when Body Gestures is enabled, a default-on left/right face-follow mode. Green boxes and persistent camera/model status make detection visible. No facial identity recognition, distance estimation, navigation or obstacle avoidance.
- Basic mode uses touch and a small keyword mapping, clearly distinguished from generative AI.
- Camera and model load only when enabled. The CPU detector processes the complete camera aspect ratio (long edge at most 480 pixels), using a 0.5 detection threshold. A visible floating preview keeps camera feedback available during scrolling and full-screen face mode. Models/runtime are downloaded from Google and jsDelivr; camera frames are not sent to the generative AI endpoint unless image sharing is enabled.
...
- Four independently addressed motors. Speeds represent percentages: the receiver converts `abs(speed) × 255 / 100` to the extension’s 0–255 output. Default cap: 50%; user-adjustable 20–80%. **Replace the Preview 01 receiver with the current receiver to apply this conversion.**
- Each motor command expires after 50–600 ms in the micro:bit receiver.
- Explicit forward, backward, left, and right commands use a deterministic 45%, 400 ms action. Text/buttons work without AI; voice requires Gemini transcription. AI-generated expressive steps remain available for other interactions.
- Face follow sends opposite left/right wheel speeds only, 24–42% for 220 ms per update. It needs two fresh detections, smooths horizontal error, and stops at the center or on loss. A mounting-direction reversal option is available.
- Voice processing and direct gestures take precedence over face follow. Follow resumes after the interaction; Stop disarms it. No forward/back movement is generated by face follow.
...
- Connection feedback, recognized speech, selected action, latest sent packet, and visible stop controls. Motor test buttons isolate movement from AI processing.
...
7. Connect the micro:bit and enable **Body Gestures** on a clear floor. First test the **Forward** button under motor checks; it runs for 0.4 seconds. Say “앞으로” while holding the microphone and release to perform the same action after transcription.
8. With the camera on and **Follow Face Left/Right** checked, move your face left and right. PONI rotates to center it, stopping when centered or when no face is detected. If the phone mount reverses the direction, use the follow-direction switch. This preview has no edge or obstacle detection.
9. Enter full-screen face mode when the phone is mounted on the robot.
...
Positive speed uses `aiPonybot.Direction.Clockwise`; negative uses `CounterClockwise`. The signed speed is a percentage; 40 becomes motor output 102, 50 becomes 128, and 80 becomes 204. The extension's existing motor direction compensation is preserved. The receiver checks the complete frame before applying motor values. Expiry and disconnect stop the motors without relying on another web command. “Sent” means a completed BLE write, not hardware execution acknowledgement.
...
| `vision.js`, `tracking.js` | Full-frame local face detection, yaw-only follow, and optional scene snapshots |
...
Automated checks cover plan validation, explicit/negated direction commands, yaw-only follow and loss, action priority, portrait coordinates, protocol bounds, interruption, WAV encoding, simulated receiver output scaling/expiry, and browser flows with mocked AI/BLE. A real MediaPipe CPU detector is checked with portrait video generated from a public face fixture, including a blank-frame negative case. Browser layout is checked at phone, tablet, desktop and landscape dimensions. Tests do not establish real API quality, real micro:bit compatibility, or iPhone/Bluefy performance.
